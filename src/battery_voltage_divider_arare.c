/*
 * arare — 電池電圧ドライバ（SAADCのオーバーサンプリングを無効化した版）
 *
 * ZMK v0.3.0 の app/module/drivers/sensor/battery/battery_voltage_divider.c を
 * MIT ライセンスのもとで複製し、compatible と oversampling だけを変更したもの。
 * 本家を fork せずに済ませるため、arare の設定リポジトリ側にドライバを持つ。
 *
 * 有効化: デバイスツリーで compatible = "arare,battery-voltage-divider" のノードを
 *   status = "okay" にし、chosen { zmk,battery = <そのノード>; } を向ける。
 *   本家の &vbatt は status = "disabled" にすること（二重定義を避ける）。
 *
 * Copyright (c) 2020 The ZMK Contributors
 * SPDX-License-Identifier: MIT
 */

#define DT_DRV_COMPAT arare_battery_voltage_divider
...
static int bvd_init(const struct device *dev) {
    ...
    drv_data->as = (struct adc_sequence){
        ...
        /* ★ここが本家との唯一の実質差分。
         * 4(=16倍オーバーサンプリング)にすると Zephyr が SAADC の BURST を有効化し、
         * アナログスティックの複数チャネル読みと切り替わる際に END イベントが出なくなって
         * adc_read が K_FOREVER で固まる。詳細は arare_L.conf のコメント。
         * 代償: 平均化が無くなるぶん電池電圧の読みが本家よりノイジーになる。 */
        .oversampling = 0,
        .calibrate = true,
    };

...
| 右手・ホイール | 軸は左右方向で、前後に転がします（マウスのスクロールホイールと同じ向き）。出荷時は回すとページ送り、押し込みで ⌘V（貼り付け）。 |
| 左手・ノブ | 軸は上向きで、ボリュームつまみのように水平に回します。出荷時は回すと拡大 / 縮小（⌘+ / ⌘-）、押し込みで &#8679;&#8984;V。 |
| 左手・スティック | トラックパッドの代わりとして載せています。2 本指・3 本指でするような操作を、親指ひとつでできるように、というつもりです。倒すとその向きのショートカットが出ます（出荷時は Mission Control とデスクトップの移動＝トラックパッドの 3 本指スワイプと同じ操作）。カーソルはトラックボールの担当です。押し込んでいるあいだは L3 レイヤーになります。 |
...
3. **キーの割り当てを変える** — 書き換える前に、右手の外側のタクトスイッチ（UNLOCK）を一回押します。→ [arare Studio](https://amearare54.github.io/arare/)
...
| 本体サイズ | 片手あたり 122.1 &#215; 96.1 mm ／ 高さ 23.8 mm（右）・28.1 mm（左・ノブ含む） |
| キーピッチ | 17 &#215; 17 mm |
...
- ZMK Studio 対応（central = 右手に USB 接続して <https://zmk.studio/> でも使えます）。タクトSW は左外側・左内側・右内側＝Bluetooth の接続先 1〜3、右外側＝アンロック
...
| エンコーダ | EC12縦 (D2/D3) 押込=Shift+Cmd+V | CKW12横 (D6/D7) 押込=Cmd+V |
| ポインタ | スティック アナログXY (AIN2/AIN3)＝ジェスチャー専用（右手の `joy_gesture` が処理）＋ 押込=D10直結キー（`&mo 3`） | PAW3222 (spi1: SCK=P1.13 SDIO=P1.14 CS=P1.15 MOTION=P0.09) |

スティックのアナログXYは `CONFIG_ZMK_BATTERY_REPORTING` と SAADC を取り合います。ZMK 本体の電池ドライバは oversampling=4 が固定で、併用すると SAADC が止まります。`arare_saadc_fix.dtsi` が本体の `&vbatt` を無効にし、oversampling=0 の arare 版ノードへ `zmk,battery` を向け替えることで両立させています（`arare_tuning_L.dtsi` から include）。

...
arare（霰）は左右分割・完全無線（BLE/ZMK）・ロープロファイル（Choc v2）の自作キーボードです。

- 実キー40（左21/右19）＋機能タクトSW4、キーピッチ17mm
- 右手: 19mmトラックボール（PAW3222）＋水平ロータリーエンコーダ（CKW12）
- 左手: ジョイスティック（Joy-Con型）＋縦型ロータリーエンコーダ（EC12）
- MCU: Seeed XIAO nRF52840 ×2、ZMK Studio対応
